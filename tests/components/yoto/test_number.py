...
async def test_display_dim_timeout(
    hass: HomeAssistant,
    mock_config_entry: MockConfigEntry,
    mock_yoto_manager: MagicMock,
) -> None:
    """Test the display dim timeout (battery saver) number entity."""
    player = _make_player()
    ...
    state = hass.states.get("number.my_yoto_display_dim_timeout")
    assert state is not None
...
async def test_shutdown_timeout(
    ...
    """Test the shutdown timeout (auto power off) number entity."""
    player = _make_player()
    ...
    state = hass.states.get("number.my_yoto_auto_power_off_timeout")
    assert state is not None
    assert state.state == "3600.0"


async def test_display_dim_timeout_unknown_when_none(
    hass: HomeAssistant,
    mock_config_entry: MockConfigEntry,
    mock_yoto_manager: MagicMock,
) -> None:
    """Test display dim timeout is unknown when not set."""
    await _setup_player(hass, mock_config_entry, mock_yoto_manager)
...
async def test_shutdown_timeout_unknown_when_none(
    hass: HomeAssistant,
    mock_config_entry: MockConfigEntry,
    mock_yoto_manager: MagicMock,
) -> None:
    """Test shutdown timeout is unknown when not set."""
    await _setup_player(hass, mock_config_entry, mock_yoto_manager)
...
async def test_set_display_dim_timeout(
    hass: HomeAssistant,
    mock_config_entry: MockConfigEntry,
    mock_yoto_manager: MagicMock,
) -> None:
    """Test setting the display dim timeout sends correct API payload."""
    player = _make_player()
    player.config.display_dim_timeout = "60"
    mock_yoto_manager.players = {PLAYER_ID: player}
    mock_config_entry.add_to_hass(hass)
    ...
    await hass.services.async_call(
        NUMBER_DOMAIN,
        SERVICE_SET_VALUE,
        {ATTR_ENTITY_ID: "number.my_yoto_display_dim_timeout", ATTR_VALUE: 120},
        blocking=True,
    )

    mock_yoto_manager.api._put_device_config.assert_called_once_with(
        mock_yoto_manager.token,
        PLAYER_ID,
        {"displayDimTimeout": "120"},
    )


async def test_set_shutdown_timeout(
    hass: HomeAssistant,
    mock_config_entry: MockConfigEntry,
    mock_yoto_manager: MagicMock,
) -> None:
    """Test setting the shutdown timeout sends correct API payload."""
    player = _make_player()
    player.config.shutdown_timeout = "3600"
    mock_yoto_manager.players = {PLAYER_ID: player}
    mock_config_entry.add_to_hass(hass)
    ...
    await hass.services.async_call(
        NUMBER_DOMAIN,
        SERVICE_SET_VALUE,
        {ATTR_ENTITY_ID: "number.my_yoto_auto_power_off_timeout", ATTR_VALUE: 7200},
        blocking=True,
    )
