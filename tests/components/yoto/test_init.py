...
async def test_unload_persists_refreshed_token(
    ...
    """Test unloading persists a changed refresh token."""
...
async def test_remove_config_entry_device_allows_stale(
    ...
    """Test removing a stale device via the UI."""
...
async def test_new_player_creates_entities_dynamically(
    ...
    """Test new player creates entities dynamically."""
...
async def test_stale_device_removed_on_poll(
    ...
    """Test stale device removed on poll."""
    player = _make_player()
...
async def test_remove_config_entry_device_blocks_active_device(
    ...
    """Test manual device removal blocked for active device."""
    mock_yoto_manager.players = {PLAYER_ID: _make_player()}
    mock_config_entry.add_to_hass(hass)
    await hass.config_entries.async_setup(mock_config_entry.entry_id)
    await hass.async_block_till_done()
